 ...
 * 주간 업적을 처리합니다.
 */
exports.processWeeklyRanking = functions
    .pubsub.schedule("0 0 * * 1") // 매주 월요일 00:00 KST
    .timeZone("Asia/Seoul")
    .onRun(async (context) => {
        ...
        try {
            ...
            teamsSnapshot.forEach(doc => {
                ...
                // 주간 점수 합산: 팀 멤버들의 weeklySuccessCount 합산
                ...
                teamRankings.push({
                    ...
                    consecutiveGoalWeeks: teamData.consecutiveGoalWeeks || 0, // 연속 달성 주차
                    weeklyGoalLP: teamData.weeklyGoalLP || 5000, // 주간 목표 LP (기본 5000)
                    achievements: teamData.achievements || {}, // 기존 업적 기록
                });
            });

            // 점수 기준으로 내림차순 정렬 (랭킹 산정)
            ...
            const ACHIEV_TEAMWORK_ID = 'teamworkExpert'; // ★ 새로 추가된 팀워크 전문가 ID ★
            const ACHIEV_TEAMWORK_REWARD_LP = 2000;
            const TEAMWORK_TARGET_WEEKS = 4; // 4주 연속

            
            // --- 3-A. 압도적 격차 업적 확인 및 처리 ---
            // (랭킹 1위와 2위 점수 차이 비교 로직 - 이전 코드와 동일)
            if (teamRankings.length >= 2) {
                ...
                if (scoreDifference >= GAP_TARGET && firstPlace.weeklyScore > 0) {
                    ...
                    rankingBatch.update(teamRef, {
                        ...
                        [`weeklyRanking.achievements.${ACHIEV_GAP_ID}`]: admin.firestore.FieldValue.serverTimestamp(),
                    });
                    batchCount++;

                    for (const memberId in firstPlace.members) {
            ...
            // 3-B. 개별 팀 업적 (퍼펙트 모닝, 위기 탈출 넘버원, 새벽의 선봉대, 팀워크 전문가) 처리
            for (let i = 0; i < teamRankings.length; i++) {
                ...
                if (isGoalAchieved) {
                    ...
                    // 4주 연속 달성 조건 확인 및 장기 업적 지급 (중복 지급 방지)
                    if (newConsecutiveWeeks >= TEAMWORK_TARGET_WEEKS && !team.achievements[ACHIEV_TEAMWORK_ID]) {
                        const ACHIEV_REWARD = ACHIEV_TEAMWORK_REWARD_LP;
                        
                        // 1. 팀 LP에 보상 추가
                        teamUpdates.teamLP = admin.firestore.FieldValue.increment(ACHIEV_REWARD);
                        
                        // 2. 장기 업적 달성 기록
                        teamUpdates[`achievements.${ACHIEV_TEAMWORK_ID}`] = admin.firestore.FieldValue.serverTimestamp();
                        
                        // 3. 팀원 전체에게 개인 LP 보상 지급
                ...
                // --- 3-B-3. 퍼펙트 모닝 (상위 3위 달성) 처리 ---
                if (rank <= RANK_TARGET && team.weeklyScore > 0) { 
                    const ACHIEV_REWARD = ACHIEV_RANKING_REWARD_LP;
                    teamUpdates.teamLP = admin.firestore.FieldValue.increment(ACHIEV_REWARD);
                    teamUpdates[`weeklyRanking.achievements.${ACHIEV_RANKING_ID}`] = admin.firestore.FieldValue.serverTimestamp();
                ...
                if (weeklyFailDaysCount < FAIL_DAY_LIMIT && Object.keys(team.members).length > 0) {
                    const ACHIEV_REWARD = ACHIEV_CRISIS_REWARD_LP;
                    
                    teamUpdates.teamLP = admin.firestore.FieldValue.increment(ACHIEV_REWARD);
                    teamUpdates[`weeklyRanking.achievements.${ACHIEV_CRISIS_ID}`] = admin.firestore.FieldValue.serverTimestamp();
                ...
                if (maxSuccessCount >= VANGUARD_SUCCESS_TARGET) {
                    ...
                    teamUpdates.teamLP = admin.firestore.FieldValue.increment(ACHIEV_REWARD_TEAM);
                    teamUpdates[`weeklyRanking.achievements.${ACHIEV_VANGUARD_ID}`] = admin.firestore.FieldValue.serverTimestamp();

                    for (const memberId in team.members) {
                        const userRef = db.collection('users').doc(memberId);
                        rankingBatch.update(userRef, {
                            userLP: admin.firestore.FieldValue.increment(ACHIEV_REWARD_TEAM)
                        });
                        batchCount++;
                    }
                    ...
                    for (const memberId of topPerformers) {
                        const userRef = db.collection('users').doc(memberId);
                        rankingBatch.update(userRef, {
                            userLP: admin.firestore.FieldValue.increment(ACHIEV_REWARD_INDIVIDUAL)
                        });
                        batchCount++;
                    }
            ...
            teamsSnapshot.forEach(teamDoc => {
                ...
                // 주간 업적 기록 및 주간 실패일 기록 초기화
                resetBatch.update(teamRef, {
                    'weeklyRanking.achievements.perfectMorning': admin.firestore.FieldValue.delete(),
                    'weeklyRanking.achievements.dominantVictory': admin.firestore.FieldValue.delete(),
                    'weeklyRanking.achievements.escapedCrisis': admin.firestore.FieldValue.delete(),
                    'weeklyRanking.achievements.dawnVanguard': admin.firestore.FieldValue.delete(),
                    weeklyFailDays: {}, 
                });
