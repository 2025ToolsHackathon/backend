...
exports.createTeam = onCall(async (data, context) => {
  ...
  if (!['public', 'private'].includes(accessType)) {
    throw new HttpsError("invalid-argument", "accessType은 'public' 또는 'private'이어야 합니다.");
  }
...
exports.kickMember = onCall(async (data, context) => {
  ...
  if (leaderId === memberIdToKick) {
    throw new HttpsError("failed-precondition", "팀장은 자신을 강퇴할 수 없습니다. 팀 탈퇴 기능을 사용하세요.");
  }
  ...
  try {
    await getFirestore().runTransaction(async (transaction) => {
      ...
      if (!teamDoc.exists) { throw new HttpsError("not-found", "팀이 존재하지 않습니다."); }
      if (!userDoc.exists) { throw new HttpsError("not-found", "강퇴할 유저를 찾을 수 없습니다."); }
      ...
      if (memberIsLeader) {
        userUpdateData.leaderOf = admin.firestore.FieldValue.arrayRemove(teamId); // 리더십 제거
      }
...
exports.delegateLeader = onCall(async (data, context) => {
  ...
  try {
    await getFirestore().runTransaction(async (transaction) => {
      ...
      if (!teamDoc.exists) { throw new HttpsError("not-found", "팀이 존재하지 않습니다."); }
      if (!newLeaderUserDoc.exists) { throw new HttpsError("not-found", "새 팀장 후보 유저를 찾을 수 없습니다."); }
...
exports.leaveTeam = onCall(async (data, context) => {
  ...
  if (!teamId) {
    ...
    await userRef.update({ 
      teamIds: admin.firestore.FieldValue.arrayRemove(null), // 안전하게 처리
  ...
  try {
    await getFirestore().runTransaction(async (transaction) => {
      ...
      transaction.update(userRef, {
        teamIds: admin.firestore.FieldValue.arrayRemove(teamId),
        leaderOf: admin.firestore.FieldValue.arrayRemove(teamId), // 팀장이 아니어도 안전하게 제거 시도
      });
    });
